...
class EncountMonsters:
  ...
  def __init__(self, monster_factory: m.MonsterFactory, monster_no_list: list):
    ...
    try:
      monster_list = [monster_factory.create_monster_by_no(int(monster_no)) for monster_no in monster_no_list]
  ...
  def construct(self, monster_list: list):
    ...
    for findability_elem, n_monster in zip(m.Findability, n_monsters):
      ret[findability_elem] = extract_findability(monster_list, findability_elem)

      if len(ret[findability_elem]) != n_monster:
        raise ValueError(f'invalid {findability_elem}')
  ...
  def judge_mass(self):
    ...
    n_very_oftens = sum(1 for monster in very_oftens if monster.exp_ratio == m.ExpRatio.P_ZERO)
    ...
    if n_very_oftens == 1:
      ...
      if all( m.exp_ratio == m.ExpRatio.P_ONE or monster.exp_ratio == m.ExpRatio.P_TWO  for monster in oftens):
        target_monster = [monster for monster in very_oftens if monster.exp_ratio == m.ExpRatio.P_ZERO][0]
    ...
    # ②よく枠に等倍がいる
    oftens = self.monster_list[m.Findability.OFTEN]
    n_oftens = sum(1 for monster in oftens if monster.exp_ratio == m.ExpRatio.P_ZERO)

    # 1匹だけいる　→大量発生
    if n_oftens == 1:
      target_monster = [monster for monster in oftens if monster.exp_ratio == m.ExpRatio.P_ZERO][0]
      return f'大量発生:{target_monster.name}'
    ...
    n_sometimes = sum(1 for monster in sometimes if monster.exp_ratio == m.ExpRatio.P_ZERO)

    # 1匹だけいる→大量発生
    if n_sometimes == 1:
      target_monster = [monster for monster in sometimes if monster.exp_ratio == m.ExpRatio.P_ZERO][0]
      return f'大量発生:{target_monster.name}'
